...
| RT-06 | Toggle AVISO Inset, SRW 1, Weather, and Timer independently. | Each actual native window changes immediately and the other three retain their state. |
...
| RT-11 | Open Insets and inspect every row/action before and after selecting the current default preset. | Visibility and a compact per-window Reset action for AVISO/SRW 1/Weather/Timer plus all preset operations are present only here; the action reads `Set default` for a non-default preset and changes to `Clear default` for the current default. |
| RT-12 | Change the position, applicable pan/zoom, and snap mode of each inset, then use its compact Reset action. | Only that inset returns to its floating default view; its visibility and the other three inset states do not change. Timer countdown state is not cleared by this layout Reset. |
...
| ID | Action | Expected result |
| --- | --- | --- |
| AI-01 | Inspect AVISO, SRW 1, Weather, and Timer in their supported floating and snapped placements. | Every inset uses the striped title bar with a coherent right-aligned `X`. AVISO, Weather, and Timer have no `F`, `P`, `R`, Editor, Reload, or Presets action; floating SRW 1 retains only its altitude-filter `F` beside `X`. The title bar is not treated as content. |
| AI-02 | Drag each resizable inset title toward every radar edge and corner, pause before release, release, then drag the snapped title again. | A clear preview appears before release and exactly matches the resulting half-screen or quadrant frame. Every edge/corner target works, and drag-to-unsnap keeps the grabbed title point under the cursor without a jump. Timer is covered separately by AI-10. |
| AI-03 | Hover and drag every resizable inset's floating edge and corner; resize each inward snapped divider; click/release each border without moving, then move onto empty radar space. | Horizontal, vertical, and both diagonal stock resize cursors appear only inside the exact forgiving drag bands. Every point showing a resize cursor starts that resize, anchored dividers stay docked, a click without a drag never nudges or unsnaps the inset, and the resize cursor clears after release when the pointer leaves the frame. Timer exposes no resize cursor or dead title-bar margin. |
...
| AI-06 | For SRW 1, right-drag to pan, wheel over several cursor positions to zoom, drag the title into each corner, drag back to float, and inspect the toolbar. | Pan and cursor-anchored zoom match AVISO behavior, corner/split snapping works, and the floating toolbar has altitude-filter `F` and close `X` only—no `Z` or `R`. |
...
| AI-10 | Verify `Release\vSMR_Data\Audio\Alarm.wav` and `Ding.wav` exist. Open Timer; left-click each idle `1M`/`2M`/`3M` cell, wait for updates, then right-click each one. Let one expire while visible, another while hidden, and arrange simultaneous expirations. Temporarily remove `Alarm.wav`, repeat an expiry, then restore it. Drag Timer near every radar edge and corner and use its Runtime Reset while a countdown is running. | The three countdowns run independently and refresh once per second; right-click resets only the selected cell. External `Alarm.wav` plays once at each expiry tick, still works while hidden, and simultaneous expirations do not restart it repeatedly. A missing file is logged without a fallback sound or crash. The fixed-size frame previews and magnet-snaps without resizing or shrinking main AVISO, and layout Reset preserves the running countdown. |

## Inset presets

| ID | Action | Expected result |
| --- | --- | --- |
| IP-01 | Arrange main AVISO view, AVISO inset, SRW 1, Weather, Timer, filters, scales, visibility, pan, and snap layout; save a uniquely named preset from the Runtime Menu. | Preset is created and becomes active. |
| IP-02 | Change all those values, then load the preset from the Runtime Menu. | Main/secondary AVISO, SRW 1, Weather, and Timer restore their captured layout state; any active Timer countdown keeps running. |
...
| IP-07 | Create distinct LFPG and LFPO presets (including SRW 1, Weather, and Timer), switch airports in both directions, and reopen the ASR. | Each airport lists and restores only its own presets/default/four-window state; Weather follows the selected ICAO, an unused airport is reset and hidden, and no LFPG name or geometry appears under LFPO. |
...
| IP-11 | Arrange Weather and Timer, then load presets created before either inset was supported. | Each legacy preset restores only the fields it contains; current Weather and Timer visibility and placement remain unchanged, and Timer countdown state is untouched. |
...
| IP-13 | Create and activate an airport preset, then create, duplicate, rename, and delete profiles while alternating the active profile. | Profile lifecycle operations never clone, rename, remove, or deactivate the airport preset; every surviving profile sees the same list and default. |
