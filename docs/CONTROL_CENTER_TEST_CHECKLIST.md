...
| ID | Action | Expected result |
| --- | --- | --- |
| AI-01 | Inspect AVISO and both SRWs in floating, edge-snapped, and corner-snapped layouts. | Every inset uses the striped title bar with a coherent right-aligned `X`. AVISO has no `F`, `P`, `R`, Editor, Reload, or Presets action; a floating SRW retains only its altitude-filter `F` beside `X`. The title bar is not treated as map content. |
