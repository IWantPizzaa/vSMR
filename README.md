...
The Symbols page previews the selected target style. NOVA shows its filled primary radar return in the configured target color together with the center marker; disabling `Show primary target` removes the filled return from both the preview and radar display.
