...
void CSMRRadar::RenderAvisoGeoJson(Gdiplus::Graphics& graphics)
{
	...
	auto drawRasterCacheTransformed = [&]() -> bool
	{
		...
		const double lonScaleRatio = lonSpan / cachedLonSpan;
		const double latScaleRatio = latSpan / cachedLatSpan;
		...
		graphics.DrawImage(
			...
			RectF(static_cast<REAL>(destX), static_cast<REAL>(destY), static_cast<REAL>(AvisoGeoJsonRasterWidth), static_cast<REAL>(AvisoGeoJsonRasterHeight)),
			0.0f,
			0.0f,
	...
	auto cacheScaleChanged = [&]() -> bool
	{
		...
		const double lonScaleRatio = lonSpan / cachedLonSpan;
		const double latScaleRatio = latSpan / cachedLatSpan;
		const double zoomTolerance = 0.0025;
		return std::abs(lonScaleRatio - 1.0) > zoomTolerance ||
			std::abs(latScaleRatio - 1.0) > zoomTolerance;
	};

	auto drawZoomPreview = [&]() -> bool
	{
		GraphicsState state = graphics.Save();
		graphics.SetClip(Rect(radarArea.left, radarArea.top, radarWidth, radarHeight), CombineModeIntersect);
		graphics.SetCompositingQuality(CompositingQualityHighSpeed);
		...
		auto appendPreviewPoint = [&](std::vector<PointF>& points, AvisoPoint& lastCoordinate, bool& hasLastCoordinate, const AvisoPoint& coordinate, bool force)
		{
			if (!force && hasLastCoordinate)
			{
				...
				if ((approxDx * approxDx + approxDy * approxDy) < minPreviewPointDistanceSquared)
					return;
			}

			const PointF point = projectPreviewPoint(coordinate);
			if (!force && !points.empty())
			{
				...
				if ((dx * dx + dy * dy) < minPreviewPointDistanceSquared)
					return;
			}
		...
		std::vector<PointF> previewPoints;
		for (const AvisoFeature& feature : AvisoGeoJsonFeatures)
		{
			...
			if (featurePixelWidth < 1.25 && featurePixelHeight < 1.25)
				continue;
			...
			if (feature.polygon)
			{
				for (const std::vector<AvisoPoint>& ring : feature.paths)
				{
					...
					previewPoints.clear();
					previewPoints.reserve(ring.size());
					AvisoPoint lastCoordinate{};
					bool hasLastCoordinate = false;
					for (size_t pointIndex = 0; pointIndex < ring.size(); ++pointIndex)
						appendPreviewPoint(previewPoints, lastCoordinate, hasLastCoordinate, ring[pointIndex], pointIndex == 0);
					...
					if (feature.fillColor.GetAlpha() > 0)
					{
						...
						graphics.FillPolygon(&fillBrush, previewPoints.data(), static_cast<INT>(previewPoints.size()), FillModeAlternate);
					}
						...
						!AvisoColorsEqual(feature.fillColor, feature.strokeColor))
					{
						...
						graphics.DrawPolygon(&outlinePen, previewPoints.data(), static_cast<INT>(previewPoints.size()));
					}
				}
				continue;
			}
			...
			for (const std::vector<AvisoPoint>& line : feature.paths)
			{
				...
				previewPoints.clear();
				previewPoints.reserve(line.size());
				AvisoPoint lastCoordinate{};
				bool hasLastCoordinate = false;
				for (size_t pointIndex = 0; pointIndex < line.size(); ++pointIndex)
					appendPreviewPoint(previewPoints, lastCoordinate, hasLastCoordinate, line[pointIndex], pointIndex == 0 || pointIndex + 1 == line.size());
	...
	const unsigned long zoomPreviewDelayMs = 180;
	if (cacheScaleChanged() &&
		(nowTick - AvisoGeoJsonLastViewChangeTick) < zoomPreviewDelayMs &&
		drawZoomPreview())
	{
		return;
	}
