#pragma once
#include <string>
#include <fstream>
#include <sstream>
#include <iostream>
#include <map>
#include <unordered_set>
#include <vector>
#include <functional>
#include <cctype>
#include <Gdiplus.h>
#include "rapidjson/document.h"
#include "rapidjson/prettywriter.h"
#include "rapidjson/stringbuffer.h"
#include "radar/RadarUiSupport.hpp"

using namespace rapidjson;

class CConfig
{
public:
	inline static constexpr size_t MaximumSerializedInputBytes =
		16U * 1024U * 1024U;

	enum class AvisoPresetTransactionAction
	{
		Abort,
		NoChange,
		Save
	};

	using AvisoPresetTransaction = std::function<AvisoPresetTransactionAction(
		rapidjson::Value& sharedMetadata,
		rapidjson::Document::AllocatorType& allocator)>;

	struct ProfileSaveIdentity
	{
		std::string currentName;
		std::string persistedName;
	};

	struct mapData
	{
		std::string element;
		std::string active; //ACTIVE:RWY:ARR:08R:DEP:08L

		bool operator==(const mapData& other) const noexcept {
			return element == other.element && active == other.active;
		}
		bool operator!=(const mapData& other) const noexcept {
			return !(*this == other);
		}
	};

	CConfig(std::string configPath, std::string mapsPath);
	virtual ~CConfig();

	const Value& getActiveProfile() const;
	Value& getMutableActiveProfile();
	bool isSidColorAvail(std::string sid, std::string airport);
	Gdiplus::Color getSidColor(std::string sid, std::string airport);
	const Value& getAirportMapIfAny(std::string airport);
	bool isAirportMapAvail(std::string airport);
	bool isCustomRunwayAvail(std::string airport, std::string name1, std::string name2);
	bool isCustomCursorUsed();

	Gdiplus::Color getConfigColor(const Value& config_path);
	COLORREF getConfigColorRef(const Value& config_path);

	std::vector<std::string> getAllProfiles() const;
	size_t getProfileCount() const;

	bool saveConfig(
		const std::vector<ProfileSaveIdentity>& profileIdentities = {},
		const std::string& expectedRevision = {},
		std::string* error = nullptr,
		bool allowRecoveryReplacement = false);
	std::string getConfigRevision() const;
	std::string getPersistedConfigRevision() const;
	std::string getLastLoadMessage() const;
	bool isConfigHealthy() const;
	static bool validateAndMigrateProfilesDocument(
		rapidjson::Document& profilesDocument,
		std::string& error,
		bool& migrated);
	static bool validateSerializedInputLimits(
		const std::string& serializedJson,
		std::string& error);
	const Value* getSharedAvisoPresetContainer() const;
	bool transactAvisoPresetStore(
		const std::string& preferredProfileName,
		const std::string& activeAirport,
		const AvisoPresetTransaction& transaction);
	bool assignUnscopedAvisoPresetsToAirport(
		const std::string& preferredProfileName,
		const std::string& airport,
		size_t& assignedPresetCount,
		std::string& error);
	bool sharesConfigFileWith(const CConfig& other) const;

	std::unordered_set<std::string> getInactiveAlert();
	bool setInactiveAlert(const std::unordered_set<std::string>& inactiveAlerts);
	std::string getLastActiveProfileName() const;
	bool setLastActiveProfileName(const std::string& profileName);

	inline int isItActiveProfile(std::string toTest) {
		auto it = profiles.find(toTest);
		if (it != profiles.end())
			return active_profile == it->second ? 1 : 0;

		for (const auto& profileEntry : profiles)
		{
			if (profileNamesEqualNoCase(profileEntry.first, toTest))
				return active_profile == profileEntry.second ? 1 : 0;
		}

		return 0;
	};

	inline void setActiveProfile(std::string newProfile) {
		const std::string trimmedProfile = trimProfileName(newProfile);

		auto it = profiles.find(trimmedProfile);
		if (it != profiles.end())
		{
			active_profile = it->second;
			return;
		}

		for (const auto& profileEntry : profiles)
		{
			if (profileNamesEqualNoCase(profileEntry.first, trimmedProfile))
			{
				active_profile = profileEntry.second;
				return;
			}
		}

		if (!profiles.empty())
			active_profile = profiles.begin()->second;
	};

	inline std::string getActiveProfileName() const {
		std::string name;
		for (std::map<std::string, rapidjson::SizeType>::const_iterator it = profiles.begin(); it != profiles.end(); ++it)
		{
			if (it->second == active_profile) {
				name = it->first;
				break;
			}
		}
		return name;
	};

	std::vector<mapData> getMapElementsForZoomLevel(int zoomLevel);

	Document document;
	Document mapDocument;
	bool reload();
	bool replaceInMemoryConfig(
		const Value& replacementDocument,
		const std::string& requestedActiveProfile,
		std::string& error);

protected:
	static bool validateAndMigratePrevalidatedProfilesDocument(
		rapidjson::Document& profilesDocument,
		std::string& error,
		bool& migrated);
	std::string config_path;
	std::string map_path;
	rapidjson::SizeType active_profile = 0;
	std::map<std::string, rapidjson::SizeType> profiles;
	std::map<int, std::vector<mapData>> maps;
	// Mutable callers receive this instance-owned fail-closed object when no
	// profile is available, never a shared/global sentinel.
	rapidjson::Value invalid_profile;
	std::string config_revision;
	std::string last_load_message;
	bool config_healthy = false;

	bool loadConfig();
	bool loadMap();
	const Value* findSidDefinition(const std::string& sid, const std::string& airport);
	const Value* findMetadata() const;
	Value& ensureMetadata();

	static std::string trimProfileName(const std::string& text)
	{
		size_t start = 0;
		while (start < text.size() && std::isspace(static_cast<unsigned char>(text[start])) != 0)
			++start;

		size_t end = text.size();
		while (end > start && std::isspace(static_cast<unsigned char>(text[end - 1])) != 0)
			--end;

		return text.substr(start, end - start);
	}

	static bool profileNamesEqualNoCase(const std::string& left, const std::string& right)
	{
		if (left.size() != right.size())
			return false;
		for (size_t i = 0; i < left.size(); ++i)
		{
			const unsigned char leftChar = static_cast<unsigned char>(left[i]);
			const unsigned char rightChar = static_cast<unsigned char>(right[i]);
			if (std::tolower(leftChar) != std::tolower(rightChar))
				return false;
		}
		return true;
	}
};
