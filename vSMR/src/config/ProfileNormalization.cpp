...
std::string VsmrProfile::NormalizeTargetIconStyle(const std::string& style)
{
...
std::string VsmrProfile::NormalizeTagDefinitionDepartureStatus(const std::string& status)
{
...
std::string VsmrProfile::NormalizeStructuredRuleSource(const std::string& source)
{
...
std::string VsmrProfile::NormalizeStructuredRuleToken(const std::string& source, const std::string& token)
{
	std::string normalizedToken = TrimAsciiWhitespaceCopy(token);
...
std::string VsmrProfile::NormalizeStructuredRuleCondition(const std::string& source, const std::string& condition)
{
	const std::string normalizedSource = NormalizeStructuredRuleSource(source);
	std::string text = TrimAsciiWhitespaceCopy(condition);
	if (text.empty())
		return "any";
...
std::string VsmrProfile::NormalizeStructuredRuleTagType(const std::string& tagType)
{
	std::string normalized = TrimAsciiWhitespaceCopy(tagType);
...
std::string VsmrProfile::NormalizeStructuredRuleStatus(const std::string& status)
{
	std::string normalized = TrimAsciiWhitespaceCopy(status);
	std::transform(normalized.begin(), normalized.end(), normalized.begin(), [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
	std::string compact;
...
std::string VsmrProfile::NormalizeStructuredRuleDetail(const std::string& detail)
{
	std::string normalized = TrimAsciiWhitespaceCopy(detail);
	std::transform(normalized.begin(), normalized.end(), normalized.begin(), [](unsigned char c) { return static_cast<char>(std::tolower(c)); });
	if (normalized.empty() || normalized == "all" || normalized == "*")
		return "any";
...
namespace
{
	...
	public:
		ProfileNormalizer(Value& value, Document::AllocatorType& storage) : profile(value), allocator(storage) {}
		bool Run()
		{
			NormalizeFilters();
			NormalizeRimcas();
			...
			NormalizeStructuredRules();
			return changed;
		}
	private:
		Value& ensureObjectMember(Value& parent, const char* key)
		{
		...
		void ensureColorMember(Value& parent, const char* key, int r, int g, int b, int a)
		{
			if (!parent.HasMember(key) || !parent[key].IsObject())
			{
				if (parent.HasMember(key))
		...
		void replaceColorMember(Value& parent, const char* key, const Value& source)
		{
			Value sourceColor;
			VsmrRapidJson::CloneJsonValue(source, sourceColor, allocator);
		...
		void ensureBoolMember(Value& parent, const char* key, bool defaultValue)
		{
		...
		void ensureIntMember(Value& parent, const char* key, int defaultValue, int minValue, int maxValue)
		{
		...
		void ensureDoubleMember(Value& parent, const char* key, double defaultValue, double minValue, double maxValue)
		{
			defaultValue = std::clamp(defaultValue, minValue, maxValue);
		...
		void ensureResolutionPresetMember(Value& parent, const char* key, const char* defaultValue)
		{
		...
		void appendCopiedDefinition(Value& targetArray, const Value& sourceArray)
		{
		...
		void ensureDefinitionArrayMember(Value& parent, const char* key, const Value* fallbackSource)
		{
		...
		void replaceDefinitionArrayMember(Value& parent, const char* key, const Value& sourceArray)
		{
			if (!sourceArray.IsArray())
				return;
			...
			if (parent.HasMember(key) && parent[key] == definitionArray) return;
			if (parent.HasMember(key))
				parent.RemoveMember(key);
			parent.AddMember(keyValue, definitionArray, allocator);
			changed = true;
		...
		void renameMemberIfPresent(Value& parent, const char* oldKey, const char* newKey)
		{
		...
		void copyBoolMemberIfPresent(Value& parent, const char* key, const Value& sourceObject)
		{
		...
		void ensureStringArrayMember(Value& parent, const char* key, const std::vector<std::string>& defaults)
		{
			bool rebuild = false;
			if (!parent.HasMember(key) || !parent[key].IsArray())
			{
		...
		void ensureIntArrayMember(Value& parent, const char* key, const std::vector<int>& defaults)
		{
			bool rebuild = false;
			if (!parent.HasMember(key) || !parent[key].IsArray())
			{
		...
		void NormalizeTargets()
		{
			...
			ensureObjectMember(targets, "departure");
			ensureObjectMember(targets, "arrival");
		...
		void NormalizeLabelColors()
		{
			...
			for (const char* key : { "departure", "arrival", "airborne", "uncorrelated" }) ensureObjectMember(labels, key);
			Value& departureLabel = labels["departure"];
			...
			{
				Value departureStatusColors;
				VsmrRapidJson::CloneJsonValue(departureLabel["status_background_colors"], departureStatusColors, allocator);
		...
		void MigrateNoStatusDefinition(Value& departureLabel)
		{
			Value& statuses = ensureObjectMember(departureLabel, "status_definitions");
			if (!statuses.HasMember("nsts") || !statuses["nsts"].IsObject()) return;
			// Replacing fields in departureLabel can relocate its status_definitions
			// member. Copy the source and remove it before changing the parent.
			Value source;
			VsmrRapidJson::CloneJsonValue(statuses["nsts"], source, allocator);
			statuses.RemoveMember("nsts");
			changed = true;
			renameMemberIfPresent(source, "definitionDetailled", "definition_detailed");
			renameMemberIfPresent(source, "definition_detailed_same_as_definition", "definition_detailed_inherits_normal");
			if (source.HasMember("definition") && source["definition"].IsArray())
				replaceDefinitionArrayMember(departureLabel, "definition", source["definition"]);
			if (source.HasMember("definition_detailed") && source["definition_detailed"].IsArray())
				replaceDefinitionArrayMember(departureLabel, "definition_detailed", source["definition_detailed"]);
			copyBoolMemberIfPresent(departureLabel, "definition_detailed_inherits_normal", source);
		}

		void NormalizeLabelDefinitions()
		{
			Value& labels = profile["labels"];
			Value& departureLabel = labels["departure"];
			Value& arrivalLabel = labels["arrival"];
			Value& airborneLabel = labels["airborne"];
			...
			MigrateNoStatusDefinition(departureLabel);
			Value& departureStatusDefinitions = departureLabel["status_definitions"];
		...
		void migrateTypeDefinitions(const char* typeKey)
		{
		...
		void migrateDefinitionArray(Value& definitionArray, const std::string& tagType, const std::string& status, const std::string& detail)
		{
		...
		void appendStructuredRule(const std::string& sourceRaw, const std::string& tokenRaw, const std::string& conditionRaw,
		...
		{
			Value& structuredRuleItems = profile["rules"]["items"];
		...
		std::string buildRuleSignature(const std::string& source, const std::string& token, const std::string& condition,
		...
		bool parseRuleColor(const Value& item, const char* key, bool& outApply, int& outR, int& outG, int& outB)
		{
			outApply = false;
			outR = 255;
			outG = 255;
...
bool VsmrProfile::Normalize(rapidjson::Value& profile, rapidjson::Document::AllocatorType& allocator)
{
	return profile.IsObject() && ProfileNormalizer(profile, allocator).Run();
}
