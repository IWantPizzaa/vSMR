...
$handlerProject = Join-Path $RepositoryRoot "vSMR\src\crash\handler\vSMRCrashHandler.vcxproj"
...
if ([string]::IsNullOrWhiteSpace($HandlerPath)) {
    $HandlerPath = Join-Path $RepositoryRoot "vSMR\src\crash\handler\bin\$Configuration\vSMRCrashHandler.dll"
}
