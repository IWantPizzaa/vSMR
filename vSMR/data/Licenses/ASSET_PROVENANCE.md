...
| AVISO airport geometry | `AVISO/<ICAO>.geojson` | 160 airport maps refreshed from dev PR #34 (commit `4819ea1`) on 2026-09-25, using official GNG layouts and current settings. LFPG retains its 89 East and 97 West arrow lines in six grouped MultiLineString features. Current hashes are recorded in `vSMR/tests/fixtures/aviso-set-20260917.json`; earlier imports and historical arrow provenance remain in Git history. Upstream GPL-3.0 license retained as `France-Ground-Layouts.txt`; local and historical source redistribution terms still require review | Verification required |
