...
- Replaced the Symbols page's generic square-like NOVA preview with a clean white filled primary-return silhouette, center marker, and `Show primary target` behavior; normalized the NOVA, Icon, and Triangle preview sizes; and shortened the aircraft-silhouette style label from `Icon (A320)` to `Icon`.
