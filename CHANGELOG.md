...
- Reworked Settings as a balanced, scroll-free Data files, Display, and Automatic updates page. Moved CPDLC connection and PDC reminders to a dedicated rail page, placed Night/Day selection beside the AVISO color editors, and kept AVISO source loading with the other data files. Moved the New, Duplicate, and Delete actions for Modes, Profiles, and Groups into larger bottom list toolbars, with a trash-can icon for deletion.
...
- Reworked AVISO Geometry and Text as matching flat group-style editors: both now use full-height group lists without redundant nested borders or summary cards, support mixed multi-group styling, and Text no longer exposes individual labels or editable label content.
